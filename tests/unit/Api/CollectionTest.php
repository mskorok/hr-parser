...
class CollectionTest extends \Codeception\TestCase\Test {
    ...
    public function testAllowedRoles() {
        ...
        $this->collection->allow(AclRoles::ADMIN, AclRoles::ADMIN);

        $this->assertEquals($this->collection->getAllowedRoles(), [AclRoles::ADMIN]);
    }

    public function testDeniedRoles() {

        $this->collection->deny(AclRoles::CUSTOMER_SUPPORT, AclRoles::CUSTOMER_SUPPORT, AclRoles::CUSTOMER_SUPPORT);

        $this->assertEquals($this->collection->getDeniedRoles(), [AclRoles::CUSTOMER_SUPPORT, AclRoles::TRAVELER]);
    }
}